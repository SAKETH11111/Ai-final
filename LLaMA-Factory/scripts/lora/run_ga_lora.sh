...
for name in "${names[@]}"; do
  ...
  PYTHONPATH=./ python src/train_bash.py --stage ga --model_name_or_path meta-llama/Meta-Llama-3-8B-Instruct --hf_hub_token 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 --do_train --save_model --dataset ${id}_Positive --dataset_dir ./data --finetuning_type lora --lora_target qkv_proj --output_dir ./saves/unlearn_bench/People/${id}/ga_lora/llama3_8b_instruct --overwrite_cache --overwrite_output_dir --cutoff_len 512 --preprocessing_num_workers 16 --per_device_train_batch_size 8 --per_device_eval_batch_size 8 --gradient_accumulation_steps 1 --lr_scheduler_type cosine --logging_steps 10 --warmup_steps 20 --save_steps 30000 --eval_steps 30000 --eval_strategy no --load_best_model_at_end false --template llama3 --learning_rate 1e-5 --num_train_epochs 3.0 --val_size 0.0000001 --plot_loss --output_result_dir ./results/unlearn_bench/People/${id}/ga_lora/llama3_8b_instruct --fp16 --eval_dataset_dir /teamspace/studios/this_studio/RWKU/LLaMA-Factory/data/RWKU/Target/${id} --target ${id} --report_to none 2>&1 | tee ./logs/unlearn_bench/People/ga_lora/llama3_8b_instruct_${id}.log
